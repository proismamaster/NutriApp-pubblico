...
if (!isset($data['user_mail']) || !isset($data['recipe_name']) || !isset($data['ingredients'])) {
...
// Immagine della ricetta (migrazione 2026-08-30_ricette_immagine.sql).
...
try {
    ...
    $sql_recipe = "INSERT INTO na_recipes (user_mail, recipe_name, `portion`, `notes`, is_favorite, creation_date, image_url) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt_recipe = $conn->prepare($sql_recipe);
    $stmt_recipe->bind_param("ssssiss", $user_mail, $recipe_name, $portion, $notes, $is_fav, $creation_date, $image_url);    $stmt_recipe->execute();

    $recipe_id = $conn->insert_id;
    $stmt_recipe->close();

    // FIX 2026-07-24 (bug reale trovato): prima si salvavano SOLO
    // recipe_id/food_name/unit/weight_g/calories/carbs/proteins/fats,
    // perdendo silenziosamente acqua/fibre/zuccheri/grassi dettagliati/
    // tutte le vitamine e i minerali ad ogni salvataggio ricetta, anche se
    // le colonne esistevano gia' in na_recipe_ingredients. Oggi l'elenco
    // arriva dal database stesso (vedi colonneIngredienteDalDb).
    ...
    $conn->commit();
    echo json_encode(["status" => "success", "message" => "Ricetta salvata con ID $recipe_id."]);
